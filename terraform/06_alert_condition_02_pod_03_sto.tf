...
# Alert condition - STO
resource "newrelic_nrql_alert_condition" "kubernetes_pod_sto_utilization" {
  name       = "K8s Cluster ${var.cluster_name} | Pods | STO"
  ...
  nrql {
    query = "FROM K8sContainerSample SELECT max(fsUsedBytes)/max(fsCapacityBytes)*100 WHERE clusterName = '${var.cluster_name}' AND fsCapacityBytes IS NOT NULL FACET podName, containerName"
  }
