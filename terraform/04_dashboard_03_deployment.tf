...
resource "newrelic_one_dashboard_raw" "kubernetes_deployment_overview" {
  ...
  name = "K8s Cluster ${var.cluster_name} | Namespace (${var.deployments[count.index].namespaceName}) | Deployments"

  ###########################
  ### DEPLOYMENT OVERVIEW ###
  ###########################
  dynamic "page" {
    for_each = var.deployments[count.index].deploymentNames
    ...
    content {
      ...
      widget {
        ...
        {
          "text": "## Deployment Overview\nNamespace -> ${var.deployments[count.index].namespaceName}\nDeployment -> ${page.value}."
        })
      }
          ...
          "nrqlQueries": [
            {
              ...
              "query": "FROM K8sContainerSample SELECT uniques(containerName) WHERE podName IN (FROM K8sPodSample SELECT uniques(podName) WHERE clusterName = '${var.cluster_name}' AND deploymentName = '${page.value}' LIMIT MAX) LIMIT MAX"
            }
          ]
        })
      }
      ...
      widget {
        ...
        configuration = jsonencode(
        {
          "nrqlQueries": [
            {
              "accountId": var.NEW_RELIC_ACCOUNT_ID,
              "query": "FROM K8sContainerSample SELECT uniqueCount(containerName) AS `Running` WHERE clusterName = '${var.cluster_name}' AND deploymentName = '${page.value}' AND status = 'Running' LIMIT MAX"
            }
          ]
        })
      }
      ...
      widget {
        ...
        configuration = jsonencode(
        {
          "nrqlQueries": [
            {
              "accountId": var.NEW_RELIC_ACCOUNT_ID,
              "query": "FROM K8sContainerSample SELECT uniqueCount(containerName) AS `Not Running` WHERE clusterName = '${var.cluster_name}' AND deploymentName = '${page.value}' AND status != 'Running' LIMIT MAX"
            }
          ]
        })
      }
      ...
      widget {
        ...
        configuration = jsonencode(
        {
          "nrqlQueries": [
            {
              "accountId": var.NEW_RELIC_ACCOUNT_ID,
              "query": "FROM K8sPodSample SELECT uniqueCount(podName) OR 0 AS `Running` WHERE clusterName = '${var.cluster_name}' AND deploymentName = '${page.value}' AND status = 'Running' LIMIT MAX"
            }
          ]
        })
      }
      ...
      widget {
        ...
        configuration = jsonencode(
        {
          "nrqlQueries": [
            {
              "accountId": var.NEW_RELIC_ACCOUNT_ID,
              "query": "FROM K8sPodSample SELECT uniqueCount(podName) OR 0 AS `Pending` WHERE clusterName = '${var.cluster_name}' AND deploymentName = '${page.value}' AND status = 'Pending' LIMIT MAX"
            }
          ]
        })
      }
      ...
      widget {
        ...
        configuration = jsonencode(
        {
          "nrqlQueries": [
            {
              "accountId": var.NEW_RELIC_ACCOUNT_ID,
              "query": "FROM K8sPodSample SELECT uniqueCount(podName) OR 0 AS `Failed` WHERE clusterName = '${var.cluster_name}' AND deploymentName = '${page.value}' AND status = 'Failed' LIMIT MAX"
            }
          ]
        })
      }
      ...
      widget {
        ...
        configuration = jsonencode(
        {
          "nrqlQueries": [
            {
              "accountId": var.NEW_RELIC_ACCOUNT_ID,
              "query": "FROM K8sPodSample SELECT uniqueCount(podName) OR 0 AS `Unknown` WHERE clusterName = '${var.cluster_name}' AND deploymentName = '${page.value}' AND status = 'Unknown' LIMIT MAX"
            }
          ]
        })
      }
